        ...
        yearSnap.forEach(function(weekSnap){
            weekSnap.forEach(function(regSnap){
                ...
                if((filtro_inges || selec_inge == reg.inge) && (filtro_obras || (selec_obra == reg.obra)) && (proc_adecuado) && (fecha_i_timestamp < reg.checkin && reg.checkin < fecha_f_timestamp)){
                    var hoy = new Date();
                    var horas = reg.status ? (parseFloat(reg.horas)/3600000).toFixed(2) : ((parseFloat(hoy.getTime()) - parseFloat(reg.checkin))/3600000).toFixed(2);;
                    ...
                    if(reg.obra != 'Otros'){
                        ...
                        horas_suma = reg.status ? 0 : ((parseFloat(hoy.getTime()) - parseFloat(reg.checkin))/3600000);
