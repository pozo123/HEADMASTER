...
$("#" + id_existente_check_ppto_proy).change(function(){
    if(this.checked){
        if($('#' + id_obra_ddl_ppto_proy + " option:selected").val() == ""){
        ...
        $('#' + id_reqs_ddl_check_ppto_proy).dropdownCheckbox("reset",myDataReq);
        $('#' + id_atn_ddl_check_ppto_proy).dropdownCheckbox("reset",myDataAtn);
        $('#' + id_nombre_ppto_proy).val('');
        $('#' + id_titulo_ppto_proy).val('');
        alcance = [];
        fecha_actual = new Date();
        document.getElementById(id_anticipo1_rb_ppto_proy).checked = true;
        document.getElementById(id_fiscales_check_ppto_proy).checked = false;
        document.getElementById(id_bancarios_check_ppto_proy).checked = false;
        $('#' + id_anticipo_ppto_proy).val('');
        var list = document.getElementById(id_entrada_lista_ppto_proy);
        while (list.firstChild) {
            list.removeChild(list.firstChild);
        }
        //AQUI
        //reset precios kaizen
    }
});

$('#' + id_proc_ddl_ppto_proy).change(function(){
    var proc = obra_global_snap.child("procesos/PC00/subprocesos/" + $('#' + id_proc_ddl_ppto_proy + " option:selected").val()).val();
    var cate = proc.categoria;
    var ppto = proc.presupuesto;
    var kaiz_ant = obra_global_snap.child("procesos/PC00/subprocesos/" + $('#' + id_proc_ddl_ppto_proy + " option:selected").val() + "/kaizen");
    ...
    $('#' + id_nombre_ppto_proy).val(ppto.nombre);
    $('#' + id_titulo_ppto_proy).val(ppto.titulo_ppto);
    $('#' + id_tiempoEntrega_ppto_proy).val(ppto.tiempoEntrega);
    document.getElementById(id_fiscales_check_ppto_proy).checked = ppto.fisc_bool;
    document.getElementById(id_bancarios_check_ppto_proy).checked = ppto.banc_bool;
    if(realParse(ppto.anticipo) == 0){
        document.getElementById(id_anticipo1_rb_ppto_proy).checked = true;
        $('#' + id_anticipo_ppto_proy).val('');
    } else if(realParse(ppto.anticipo) == 100){
        document.getElementById(id_anticipo3_rb_ppto_proy).checked = true;
        $('#' + id_anticipo_ppto_proy).val('');
    } else {
        document.getElementById(id_anticipo2_rb_ppto_proy).checked = true;
        $('#' + id_anticipo_ppto_proy).val(ppto.anticipo);
    }
    ...
    $("#" + id_atn_ddl_check_ppto_proy).dropdownCheckbox("reset", myNewDataAtn);
    ...
    $("#" + id_reqs_ddl_check_ppto_proy).dropdownCheckbox("reset", myNewDataReq);
    ...
    for(key in ppto.json_alcance){
        var node = document.createElement("LI");
        node.classList.add("list-group-item");// Create a <li> node
        var textnode = document.createTextNode(ppto.json_alcance[key].texto + "\n   - Precio: " + formatMoney(parseFloat(ppto.json_alcance[key].precio)));// Create a text node
        node.appendChild(textnode);// Append the text to <li>
        document.getElementById(id_entrada_lista_ppto_proy).appendChild(node);  
        alcance.push({
    ...
    //AQUI
    horas_ppto = realParse(obra_global_snap.child("procesos/PC00/subprocesos/" + $('#' + id_proc_ddl_ppto_proy + " option:selected").val() + "/SCORE/total_prog").val());
    var total_proy = realParse(kaiz_ant.child("PROYECTOS/PPTO").val());
    $('#' + id_hora_score_ppto_proy).val(total_proy / horas_ppto);
    $('#' + id_indirectos_ppto_proy).val(realParse(obra_global_snap.child("procesos/PC00/subprocesos/" + $('#' + id_proc_ddl_ppto_proy + " option:selected").val() + "/porcentaje_indirectos").val()));
    $('#' + id_proyectos_ppto_proy).val(total_proy);
    $('#' + id_suministros_ppto_proy).val(kaiz_ant.child("PRODUCCION/SUMINISTROS/CUANT").val());
    $('#' + id_copeo_ppto_proy).val(kaiz_ant.child("PRODUCCION/COPEO/COPEO").val());
    $('#' + id_precio_venta_ppto_proy).val(realParse(kaiz_ant.child("ADMINISTRACION/ESTIMACIONES/PPTO").val()) + realParse(kaiz_ant.child("ADMINISTRACION/ANTICIPOS/PPTO").val()));
    loadProfitsPptoProy();
});
...
$('#' + id_vistaPrevia_button_ppto_proy).click(function () {
    if((document.getElementById(id_existente_check_ppto_proy).checked && $('#' + id_proc_ddl_ppto_proy + " option:selected").val() == "") || !$('#' + id_nombre_ppto_proy).val() || $('#' + id_obra_ddl_ppto_proy + " option:selected").val() == "" ||  $('#' + id_categoria_ddl_ppto_proy + " option:selected").val() == "" || alcance.length == 0){
        ...
        var ppto = generaParamPptoProy(false);
...
$('#' + id_registrar_button_ppto_proy).click(function () {
    if((document.getElementById(id_existente_check_ppto_proy).checked && $('#' + id_proc_ddl_ppto_proy + " option:selected").val() == "") || !$('#' + id_nombre_ppto_proy).val() || $('#' + id_obra_ddl_ppto_proy + " option:selected").val() == "" ||  $('#' + id_categoria_ddl_ppto_proy + " option:selected").val() == "" || alcance.length == 0){
        ...
        var ppto = generaParamPptoProy(true);
        var pdfPresupuesto = ppto[0];
        ...
        pdfDocGenerator.getDataUrl((data) => {
            ...
            if(document.getElementById(id_anticipo1_rb_ppto_proy).checked === true){
                ...
                ant = realParse($('#' + id_anticipo_ppto_proy).val());
            }
            else if(document.getElementById(id_anticipo3_rb_ppto_proy).checked === true){
                ant = 100;
            }
            var reqs_array = $('#' + id_reqs_ddl_check_ppto_proy).dropdownCheckbox("checked");
            var reqs_lista = {};
            for(i=0;i<reqs_array.length;i++){
                reqs_lista[i] = reqs_array[i].label;
            }
            var atn_array = $('#' + id_atn_ddl_check_ppto_proy).dropdownCheckbox("checked");
            var atn_lista = {};
            for(i=0;i<atn_array.length;i++){
                atn_lista[i] = atn_array[i].label;
            }
            var exc_array = $('#' + id_excs_ddl_check_ppto_proy).dropdownCheckbox("checked");
            var exc_lista = {};
            ...
            for(i=0;i<alcance.length;i++){
                alcance_json[i] = alcance[i];
            }
            var ppto_db = {
                nombre: $('#' + id_nombre_ppto_proy).val(),
                terminado: false,
                fisc_bool: document.getElementById(id_fiscales_check_ppto_proy).checked,
                banc_bool: document.getElementById(id_bancarios_check_ppto_proy).checked,
                imagenes: '',
                fecha_ppto: fecha_actual.getTime(),
                titulo_ppto: $('#' + id_titulo_ppto_proy).val(),
...
function generaParamPptoProy(genera){
    var dir = obra_global.direccion;
    var clien = obra_global.cliente;
    var titulo_ppto = $('#' + id_titulo_ppto_proy).val();
    var tiempoEntrega = $('#' + id_tiempoEntrega_ppto_proy).val();
    var fisc_bool = document.getElementById(id_fiscales_check_ppto_proy).checked == true
    var banc_bool = document.getElementById(id_bancarios_check_ppto_proy).checked == true
    var clave_presu;
...
function generaPptoProy(genera, clave_presu, obra_ppto, alcance_ppto, fecha_ppto, titulo_ppto, tiempoEntrega, fisc_bool, banc_bool, reqs_lista, exc_lista, atn_lista, anticipo){
    ...
    var logo;
    if(genera){
        obra_nombre = obra_ppto.nombre.toUpperCase();
        logo = {
    ...
    var alcance_pdf = [];
    var alcance_string = "";
    for(i = 0; i < alcance_ppto.length; i++){
        var num = i+1;
        alcance_string = alcance_string + alcance_ppto[i].texto + '. ';
        alcance_pdf[i] = [
            {  
            ...
            {  
                border: [true, false, true, false],
                text:formatMoney(alcance_ppto[i].precio),
                margin: [0,5],
                alignment: 'center',
                fontSize:8,
            }
        ];
        precio_total += parseFloat(alcance_ppto[i].precio);                 
    }
    ...
    var bod = [
        ...
        [
            ...
            },
            {
                rowSpan:4,
                border: [false, false, false, false],
                text: "Calle: " +  dir.calle + ", No. " + dir.numero + "\n" +
                "COL. " + dir.colonia + "\n" + dir.delegacion + ", \n" + 
                dir.ciudad,
                margin: [0,5],
                fontSize:8,
        ...
        [
            {  
                ...
                text: ['A continuacion enviamos a su amable consideración el presupuesto ',
                 {text: titulo_ppto.toUpperCase(), bold: true},
    ...
    for(i=0;i<alcance_pdf.length;i++){
        bod.push(alcance_pdf[i]);
    }
    bod.push([
            {  
                colSpan: 6,
