...
//Changes para calculo de la parte de utilidad AQUI
//on change del check existente, cargar ddl, contemplar si no hay obra seleccionada AQUI

$('#' + id_registrar_button_ppto_proy).click(function () {
    if(!$('#' + id_nombre_ppto_proy).val() || $('#' + id_obra_ddl_ppto_proy + " option:selected").val() == "" ||  $('#' + id_categoria_ddl_ppto_proy + " option:selected").val() == "" || alcance.length == 0){
        ...
        if(!existente){
            ...
            var num_proc = parseInt(obra_global.child("procesos/PC00/num_procesos") + 1;//AQUI cambiar PC00 por ADIC         
        ...
        var consecutivo;//AQUI chance mantener, como guardar las distintas versiones?
        //AQUI checar clave_presu, num_consec, p, consecutivo y consecutivo_str, creo que el query sobra
        //AQUI lo que necesito esta en "AQUI2", de hecho segun yo se puede hacer todo ahi.
        firebase.database().ref(rama_bd_obras + "/" + obra_selec.nombre + "/presupuestos/" + $('#' + id_nombre_presupuesto).val()).once('value').then(function(snapshot){
            ...
            var pdfPresupuesto = {
                ...
                footer: { fontSize:8,alignment: 'center', text:'Av. Constituyentes 561 Int. 101a, Col. América, Miguel Hidalgo, Ciudad de México, C.P. 11820, Tel. 6273 7900 , email: proyectos@headingenieria.mx'},//AQUI checar email
                header:function(currentPage, pageCount) {
                    return{
                        columns: [
                            {
                                //AQUI si vista previa, en vez de image:
                                //text: "NO VÁLIDO",
                        ...
                        table:{
                            ...
                            body:[
                                ...
                                [
                                    ...
                                    {
                                        border: [false, false, false, false],
                                        //AQUI si vista previa text: "DOCUMENTO NO VÁLIDO",
                                        text: obra_global.nombre.toUpperCase(),
                                        bold: true,
                                        color:'#2C3F5A',
                                        margin: [0,5],
                                        fontSize:10,
...
/*AQUI lo de ray:
Únicamente se podrá facturar IVA EXENTO si cumple con lo siguiente:
...
b.    El número de permiso, licencia o autorización de construcción correspondiente que le haya proporcionado el prestador de los servicios parciales de construcción, el cual deberá coincidir con el señalado en la manifestación a que se refiere la fracción siguiente de este artículo.*/ + '\n' + '',
            ...
            pdfDocGenerator.download(clave_presu+'.pdf')//AQUI cambiar nombre si vista previa

            //AQUI no si vista_previa
            //AQUI necesito consecutivo (num de archivos si existente) y subp (num_subprocesos +1). Segun yo puedo hacer el query aqui. AQUI2
            pdfDocGenerator.getDataUrl((data) => {
                ...
                //AQUI poner valores de kaizen y trickle down
                //AQUI llenar todos los datos
                    var subproc = {
                        alcance: ,
                        categoria: ,
                        clave: ,
                        fecha_inicio: ,
                        fecha_final: ,
                        kaizen: kaiz,//AQUI modificarlo con valores, pero aguas que no modifiques tu var kaiz
                        terminado: false,
                        contrato: "",
                    ...
                    //AQUI subp es el numero siguiente de subprocesos
...
//AQUI hacer lo mismo pero para vista previa, hacer lo de arriba una funcion