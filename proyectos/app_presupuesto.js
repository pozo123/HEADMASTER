...
function loadProfitsPptoProy(){
    ...
    var precio = parseFloat($('#' + id_precio_venta_ppto_proy).val() == "" ? 0 : $('#' + id_precio_venta_ppto_proy).val());
...
$("#" + id_profit_cantidad_ppto_proy).change(function(){
    var costos = parseFloat($('#' + id_proyectos_ppto_proy).val()) + parseFloat($('#' + id_copeo_ppto_proy).val()) + parseFloat($('#' + id_suministros_ppto_proy).val());
    $('#' + id_precio_venta_ppto_proy).val((parseFloat($("#" + id_profit_cantidad_ppto_proy).val()) + costos)/(1-porcentaje_indirectos));
    $('#' + id_profit_porcentaje_ppto_proy).val(100*parseFloat($("#" + id_profit_cantidad_ppto_proy).val())/(parseFloat($('#' + id_precio_venta_ppto_proy).val()) - parseFloat($("#" + id_profit_cantidad_ppto_proy).val())));
    highLight(id_precio_venta_ppto_proy);
    highLight(id_profit_porcentaje_ppto_proy);
});
...
$('#' + id_registrar_button_ppto_proy).click(function () {
    if((document.getElementById(id_existente_check_ppto_proy).checked && $('#' + id_proc_ddl_ppto_proy + " option:selected").val() == "") || !$('#' + id_nombre_ppto_proy).val() || $('#' + id_obra_ddl_ppto_proy + " option:selected").val() == "" ||  $('#' + id_categoria_ddl_ppto_proy + " option:selected").val() == "" || alcance.length == 0){
        ...
        pdfDocGenerator.getDataUrl((data) => {
            ...
            if(document.getElementById(id_existente_check_ppto_proy).checked){
                //AQUI modificar valores en kaizen -> calc diferencia y trickle down en pc00 y en obra(totales)
                var nombre = $('#' + id_nombre_ppto_proy).val();
                //Aqui checar si jala asi o hay que hacer un for
                var consecutivo = obra_global.procesos.PC00.presupuesto.archivos.length;//AQUI diferente para adic
                //AQUI cambiar query para ADIC
                //firebase.database().ref(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/subprocesos/" + $('#' + id_proc_ddl_ppto_proy + " option:selected").text() + "/presupuesto/nombre").set(nombre);
                //firebase.database().ref(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/subprocesos/" + $('#' + id_proc_ddl_ppto_proy + " option:selected").text() + "/presupuesto/archivos/" + consecutivo).set(cons);
            } else {
            //AQUI trickle down de kaizen
                ...
                kaiz_nuevo.PROFIT.PROG.REAL = real_kaiz;
                console.log(kaiz);
                var subproc = {
                    alcance: ppto[3],
                    categoria: clave_cat,//AQUI no si adic
                    clave: subp,
                    fecha_inicio: "",
                    fecha_final: "",
                    kaizen: kaiz_nuevo,
                ...
                //AQUI cambiar query por ADIC y descomentar
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/PROYECTOS/PPTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/PRODUCCION/SUMINISTROS/CUANT", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/PRODUCCION/COPEO/PREC", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/ADMINISTRACION/ESTIMACIONES/PPTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/ADMINISTRACION/ANTICIPOS/PPTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/PROFIT/PROG/BRUTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/procesos/PC00/kaizen/PROFIT/PROG/REAL", )

                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/PROYECTOS/PPTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/PRODUCCION/SUMINISTROS/CUANT", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/PRODUCCION/COPEO/PREC", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/ADMINISTRACION/ESTIMACIONES/PPTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/ADMINISTRACION/ANTICIPOS/PPTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/PROFIT/PROG/BRUTO", )
                //sumaEnFirebase(rama_bd_obras + "/" + obra_global.nombre + "/kaizen/PROFIT/PROG/REAL", )
