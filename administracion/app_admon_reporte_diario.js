var id_tab_reporte_diario = "tabReporteDiario";
...
$('#' + id_tab_reporte_diario).click(function() {
    inges = {};
    obras = {};
    var horas_suma = 0;
    var hoy = new Date();
    var gwhoy = getWeek(hoy.getTime());
    firebase.database().ref(rama_bd_personal).once('value').then(function(snapshot){
        snapshot.forEach(function(childSnap){
            var inge = childSnap.val();
            if(childSnap.child("nickname").val() != "SCORE" && childSnap.child("areas/proyectos").val() && !childSnap.child("areas/administracion").val() && childSnap.child("activo").val()){
                inges[childSnap.key] = {
                    nombre: inge.nickname,
                    //status: inge.status,
                    regs: {},
                    num_regs: 0,
                };
        ...
        firebase.database().ref(rama_bd_registros + "/" + gwhoy[1] + "/" + gwhoy[0]).once('value').then(function(snapshot){
            snapshot.forEach(function(regSnap){
                ...
                if(new Date(reg.checkin).toLocaleDateString("es-ES",options) == hoy.toLocaleDateString("es-ES",options)){
                    ...
                    inges[reg.inge]["regs"][inges[reg.inge]["num_regs"]] = regis;
                    inges[reg.inge]["num_regs"]++;
        
                    if(reg.obra != 'Otros'){
                        if(!obras[reg.obra]){
                            obras[reg.obra] = {};      
                        }
                        if(!obras[reg.obra]["procesos"]){
                            obras[reg.obra]["procesos"] = {};     
                        }

                        var horas_suma = reg.status ? 0 : ((parseFloat(hoy.getTime()) - parseFloat(reg.checkin))/3600000);

                        obras[reg.obra]["procesos"][reg.proceso] = obras[reg.obra]["procesos"][reg.proceso] ? (parseFloat(obras[reg.obra]["procesos"][reg.proceso]) + parseFloat(horas_suma)).toFixed(2) : (parseFloat(horas_suma)).toFixed(2); 
                    }
                }

            });
            generateReporteDiario();
        });
    });
});

function generateReporteDiario(){
    var regs = [];
    regs[0] = [{text:"Nombre", style:"tableHeader"},{text:"Obra", style:"tableHeader"},{text:"Proceso", style:"tableHeader"},{text:"Horas", style:"tableHeader"},{text:"Firma", style:"tableHeader"}];
    var renglones = [false,true];
    for(key in inges){
        if(inges[key]["num_regs"] == 0){
            regs.push([inges[key]["nombre"],"-","-","-",{table:{widths: ['*'],heights: [20],body: [['']]},}]);
            renglones.push(true);
        } else {
            if(inges[key]["num_regs"] == 1){
                regs.push([{text: inges[key]["nombre"], noWrap: true},
                    {text: inges[key]["regs"]["0"]["obra"], noWrap: true},
                    {text: inges[key]["regs"]["0"]["proc"], noWrap: true},
                    {text: inges[key]["regs"]["0"]["horas"], noWrap: true},
                    {table: {widths: ['*'],heights: [20],body: [['']]},}
                ]);
                renglones.push(true);
            } else {
                for(keyReg in inges[key]["regs"]){
                    if(keyReg == inges[key]["num_regs"] - 1){
                        renglones.push(true);
                    } else {
                        renglones.push(false);
                    }
                    console.log(inges[key]["regs"])
                    if(keyReg == 0){
                        regs.push([{text: inges[key]["nombre"], noWrap: true},
    ...
    var procs = [];
    procs[0] = [{text:"Obra", style:"tableHeader"},{text:"Proceso", style:"tableHeader"},{text:"Horas Programadas", style:"tableHeader"},{text:"Horas Trabajadas", style:"tableHeader"}];

    firebase.database().ref(rama_bd_obras).once('value').then(function(snapshot){
        for(obra in obras){
            for(proceso in obras[obra]["procesos"]){
                path = proceso.split("-");
                var query = obra + "/procesos/" + path[0];
                if (path.length > 1){
                    query = query + "/subprocesos/" + proceso
                }
                var horas_programadas = isNaN(parseFloat(snapshot.child(query + "/SCORE/total_prog").val())) ? 
                 0 : snapshot.child(query + "/SCORE/total_prog").val();
                ...
                procs.push([
                    {text: obra},
                    {text: proceso},
                    {text: horas_programadas},//,},
                    {text: horas_trabajadas_tot},
                ])
            }
        }
        doc = {     
            content: [
                ...
                {text: new Date().toLocaleDateString(), style: 'subheader',alignment: 'center'},
                " ",
                " ",
                {
                    style: 'tableExample',
                    ...
                    layout: {
                        ...
                        hLineColor: function (i, node) {
                            ...
                            if( i == 1){
                                ...
                                color = (renglones[i] || i === node.table.body.length) ? 'gray' : 'white';
                            }
                            return color
                        },
                        //vLineColor: 'black',
                ...
                {text: 'Horas programadas vs Horas trabajadas hasta hoy', style: 'subheader',alignment: 'justify', pageBreak: "before"},
                {
                    style: 'tableExample',
                    table: {
                        widths: ["auto",200,'*','*'],
